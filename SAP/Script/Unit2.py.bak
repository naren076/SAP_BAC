...
def sap_test():
  ...
  while not Driver.EOF():
    ...
    if(frame2.WaitAliasChild("frameC104", 30000).Exists): 
    ...
    if(form.WaitAliasChild("panelSpreadsheetCtrlShiftF7", 10000).Exists):
      form.FindElement("//div[@title='Spreadsheet... (Ctrl+Shift+F7)']").Click()
    #page.WaitConfirm(5000)
    if(frame.WaitAliasChild("panelUpdowndialogchoose", 10000).Exists):
